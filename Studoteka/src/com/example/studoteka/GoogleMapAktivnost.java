...
public class GoogleMapAktivnost extends FragmentActivity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		fakultet = (FakultetModel) getIntent().getSerializableExtra("fakulteti");
		...
		try{
			if(googleMap == null){
				...
				googleMap.addMarker(new MarkerOptions().position(lokacija).title(fakultet.getNaziv()).icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_AZURE)));
	...
	public void onClickRestorani(View v){
		Uri intentRestorani = Uri.parse("geo:0,0?q=restaurants,"+ fakultet.getNaziv());
	...
	public void onClickKulturniSadrzaj(View v){
		Uri intentRestorani = Uri.parse("geo:0,0?q=theaters,"+ fakultet.getNaziv());
		Intent mapIntent = new Intent(Intent.ACTION_VIEW, intentRestorani);
		mapIntent.setPackage("com.google.android.apps.maps");
		startActivity(mapIntent);
	}
